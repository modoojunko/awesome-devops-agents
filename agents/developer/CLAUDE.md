...
3. **确认 AGENT.md** 存在（`agents/developer/AGENT.md`），初始化 `.agent/pipeline-status.json`
   - 重置 pipeline 状态为 IDLE

4. **配置 GitHub Token**（如果 `gh auth status` 未通过）
   - 提示开发者运行 `bash scripts/setup-github.sh`
   - 或手动创建 GitHub Personal Access Token 并执行 `gh auth login`
   - 重试验证直到通过

5. **输出"环境就绪"**，询问开发者想处理哪个 Jira 任务
